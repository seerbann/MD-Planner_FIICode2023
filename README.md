# MD Planeer

## 👌 Choose a language

* [En](#engleza)
* [Ro](#romana)

<div id="engleza"></div>

## :bookmark_tabs: Menu

* [Technologies](#tehnologii)
* [Login & Sign In](#inregistrare)
* [Invitation for users](#invitatie)
* [Medic's app](#medic)
* [Pacient's app](#pacient)
* [Design](#design)

<div id="tehnologii"></div>

## 🖥️ Technologies

In the development of the application, we used **Flutter** for the frontend, with the help of which we were able to create both a **website** and a **mobile application**.
For the backend we used **firebase**.

<div id="inregistrare"></div>

## :closed_lock_with_key: Login & Sign In

Doctors can create an account in the app by accessing the "Register as a doctor" button, available on the first page of the application.

![firstPage](https://user-images.githubusercontent.com/72700839/228334007-233fba4a-005f-4379-8564-9436be7039ac.png)

Then they should enter information such as: First Name, Last Name, Email, Password, Phone Number, City and a Photo certifying the profession.

![inreg](https://user-images.githubusercontent.com/72700839/228334612-6e4e4d55-a5ab-466d-872f-3fcb3a93e7e1.png)

Both doctors and patients can log in using the following page:

![auten](https://user-images.githubusercontent.com/72700839/228335574-7a792ec0-ca62-46e4-a14b-9506eac11ca9.png)


<div id="invitatie"></div>


## :email: Invitation for users

To create patient accounts, instead, they must receive an invitation from the family doctor.
This will contain a dynamic link that will redirect them to the registration page.

![Trimite2](https://user-images.githubusercontent.com/72700839/228336747-407d6123-8f68-4374-bd4f-d6e0695f72e1.png)

![mail](https://user-images.githubusercontent.com/72700839/228338751-d077c948-098c-43a8-a9c5-569ce7278179.png)

![textMail](https://user-images.githubusercontent.com/72700839/228338926-9fafdef1-0095-44b3-8e38-a55fd87a6387.png)

![Screenshot 2023-03-28 220243](https://user-images.githubusercontent.com/72700839/228341486-0266a851-7bc8-478b-a33c-c180adb11929.png)

![InkedScreenshot 2023-03-28 2203202](https://user-images.githubusercontent.com/72700839/228341546-2b479cd1-0ef4-4231-978a-c85322bb7e78.jpg)


Both links will redirect to the registration page

![inregistrare](https://user-images.githubusercontent.com/72700839/228339794-f465e905-06ae-4abe-8878-96990c2c4a3b.png)


<div id="medic"></div>

## :syringe: Medic's app

The doctor's profile: information about the doctor, the possibility to send an invitation and edit the profile.

![profilMedic](https://user-images.githubusercontent.com/72700839/228348400-69dc60da-15d7-475a-aa5c-99a4f48071a4.png)

The page for profile editing

![editProfilMedic](https://user-images.githubusercontent.com/72700839/228348425-10f11e27-d88f-4070-b083-dae070254a62.png)

List of patients: You can find out information about patients, add a medical record or see one.

![listaMedic](https://user-images.githubusercontent.com/72700839/228348454-4960d361-bbb2-44fa-8730-b6e9cab152e2.png)

Viewing a medical record with a pdf viewer:

![FisaMedicala](https://user-images.githubusercontent.com/72700839/228348549-82c14e84-4b6f-43cf-82ea-4f0dfc7ab8ca.png)

Calendar: Here the doctor can see on which days he has appointments, as well as information about them.

![calendarMedic](https://user-images.githubusercontent.com/72700839/228348572-10b1e254-b278-4804-b365-15e1b7132079.png)


<div id="pacient"></div>

## :mask: Pacient's app

Patient profile: information about the patient, as well as the possibility to edit their profile.

![profilPacient](https://user-images.githubusercontent.com/72700839/228349138-d0b8ca41-394a-4b61-9763-5453b6591e40.png)

Profile edit page:

![editProfilUser](https://user-images.githubusercontent.com/72700839/228349180-ca1be34c-451d-44ab-b83c-5937dc9f3a27.png)

List of doctors: The patient can see all the doctors and can move to one of them.

![listUser](https://user-images.githubusercontent.com/72700839/228349226-3f97654a-27d2-4360-beea-f0534f882af3.png)

Calendar: The patient can choose a day and time to make an appointment with the family doctor.

![programareUser](https://user-images.githubusercontent.com/72700839/228349261-fcfc753c-603e-4652-89c6-aef7b40e7a31.png)


<div id="design"></div>

## :art: Design

- [x] The app/website is responsive.

![1](https://user-images.githubusercontent.com/72700839/228344061-b22e95ae-0aa0-4018-9bbc-268f2bd910e8.png)

![2](https://user-images.githubusercontent.com/72700839/228344086-d08304d6-a6ae-4a37-980d-0d21c6af8ae8.png)

![3](https://user-images.githubusercontent.com/72700839/228344125-128dbd7c-78cf-4621-bfb1-cafca565732f.png)

- [x] We used a color pallete

- [x] The app has an 'identity': name, logo, motto etc.

- [x] The mobile application can be used in both portrait and landscape mode.

![landscape](https://user-images.githubusercontent.com/72700839/228344999-3d27d941-fb7b-4d0c-8edf-7f6691bd9cbe.png)







## Team Name : fluTTeam

![logo2](https://user-images.githubusercontent.com/72700839/228325427-73e8158b-8870-407e-a133-103f5406686b.jpg)

Echipa formata din **Chiriac Serban-Mihail** & **Stefan Vlad**


### Video:

[![](https://markdown-videos.deta.dev/youtube/bYQXuKlLhyg)](https://youtu.be/bYQXuKlLhyg)

[Link](https://youtu.be/bYQXuKlLhyg)

<div id="romana"></div>

## :bookmark_tabs: Meniu

* [Tehnologii folosite](#tehnologii)
* [Inregistrare & Autentificare](#inregistrare)
* [Invitatie pentru user](#invitatie)
* [Aplicatia medicului](#medic)
* [Aplicația pacientului](#pacient)
* [Cerinte design](#design)

<div id="tehnologii"></div>

## 🖥️ Tehnologii folosite

In dezvoltarea aplicatiei am folosit pentru frontend **Flutter**, cu ajutorul caruia am putut realiza atat un **site web**, cat si o **aplicatie pentru mobil**.
Pentru backend am folosit **firebase**.

<div id="inregistrare"></div>

## :closed_lock_with_key: Inregistrare & Autentificare

Medicii isi pot crea un cont in aplicatie accesand butonul "Inregistreaza-te ca medic", disponibil in prima pagina a aplicatiei.

![firstPage](https://user-images.githubusercontent.com/72700839/228334007-233fba4a-005f-4379-8564-9436be7039ac.png)

Apoi acestia ar trebui sa introduca informatii precum: Prenume, Nume, Email, Parola, Numar de telefon, Orasul de unde este si o Poza ce certifica profesia.

![inreg](https://user-images.githubusercontent.com/72700839/228334612-6e4e4d55-a5ab-466d-872f-3fcb3a93e7e1.png)

Atat medicii cat si pacientii se pot autentifica utilizand urmatoarea pagina:

![auten](https://user-images.githubusercontent.com/72700839/228335574-7a792ec0-ca62-46e4-a14b-9506eac11ca9.png)


<div id="invitatie"></div>


## :email: Invitatie pentru user

Pentru crearea conturilor de pacienti, in schimb, acestia trebuie sa primeasca o invitatie din partea medicului de familie.
Aceasta va contine un link dinamic ce ii va redirectiona catre pagina de inregistrare.

![Trimite2](https://user-images.githubusercontent.com/72700839/228336747-407d6123-8f68-4374-bd4f-d6e0695f72e1.png)

![mail](https://user-images.githubusercontent.com/72700839/228338751-d077c948-098c-43a8-a9c5-569ce7278179.png)

![textMail](https://user-images.githubusercontent.com/72700839/228338926-9fafdef1-0095-44b3-8e38-a55fd87a6387.png)

![Screenshot 2023-03-28 220243](https://user-images.githubusercontent.com/72700839/228341486-0266a851-7bc8-478b-a33c-c180adb11929.png)

![InkedScreenshot 2023-03-28 2203202](https://user-images.githubusercontent.com/72700839/228341546-2b479cd1-0ef4-4231-978a-c85322bb7e78.jpg)


Ambele link-uri vor redirectiona catre pagina de inregistrare

![inregistrare](https://user-images.githubusercontent.com/72700839/228339794-f465e905-06ae-4abe-8878-96990c2c4a3b.png)


<div id="medic"></div>

## :syringe: Aplicatia medicului

Profilul medicului: informatii despre medic, posibilitatea de a trimite o invitatie si de a-si edita profilul.

![profilMedic](https://user-images.githubusercontent.com/72700839/228348400-69dc60da-15d7-475a-aa5c-99a4f48071a4.png)

Pagina de editare a profilului:

![editProfilMedic](https://user-images.githubusercontent.com/72700839/228348425-10f11e27-d88f-4070-b083-dae070254a62.png)

Lista cu pacientii: Poti afla informatii despre pacienti, le poti adauga o fisa medicala sau de a vedea o astfel de fisa.

![listaMedic](https://user-images.githubusercontent.com/72700839/228348454-4960d361-bbb2-44fa-8730-b6e9cab152e2.png)

Vizualizarea unei fise medicale:

![FisaMedicala](https://user-images.githubusercontent.com/72700839/228348549-82c14e84-4b6f-43cf-82ea-4f0dfc7ab8ca.png)

Calendar: Aici medicul poate vedea in ce zile are programari, cat si informatii despre acestea.

![calendarMedic](https://user-images.githubusercontent.com/72700839/228348572-10b1e254-b278-4804-b365-15e1b7132079.png)


<div id="pacient"></div>

## :mask: Aplicatia pacientului

Profilul pacientului: informatii despre pacient, cat si posibilitatea de a-si edita profilul.

![profilPacient](https://user-images.githubusercontent.com/72700839/228349138-d0b8ca41-394a-4b61-9763-5453b6591e40.png)

Pagina de editare a profilului:

![editProfilUser](https://user-images.githubusercontent.com/72700839/228349180-ca1be34c-451d-44ab-b83c-5937dc9f3a27.png)

Lista cu medicii: Pacientul poate vedea medicii din apropriere si se poate muta la unul dintre acestia.

![listUser](https://user-images.githubusercontent.com/72700839/228349226-3f97654a-27d2-4360-beea-f0534f882af3.png)

Calendar: Pacientul poate alege o zi si o ora in care sa se programeze la medicul de familie.

![programareUser](https://user-images.githubusercontent.com/72700839/228349261-fcfc753c-603e-4652-89c6-aef7b40e7a31.png)


<div id="design"></div>

## :art: Cerinte design

- [x] Aplicatia este responsive.

![1](https://user-images.githubusercontent.com/72700839/228344061-b22e95ae-0aa0-4018-9bbc-268f2bd910e8.png)

![2](https://user-images.githubusercontent.com/72700839/228344086-d08304d6-a6ae-4a37-980d-0d21c6af8ae8.png)

![3](https://user-images.githubusercontent.com/72700839/228344125-128dbd7c-78cf-4621-bfb1-cafca565732f.png)

- [x] Am folosit o tema de culori în construirea celor aplicatiei

- [x] Identitatea aplicației: nume, logo, motto etc.

- [x] Aplicatia de pe mobil poate fi folosita atat in mod portret, cat si landscape.

![landscape](https://user-images.githubusercontent.com/72700839/228344999-3d27d941-fb7b-4d0c-8edf-7f6691bd9cbe.png)

